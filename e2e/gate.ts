 ...
 * plausible offender: it prints hex tags, a MAC comparison table and a timing
 * measurement table, all inside `.table-wrap` scrollers.
 */
 ...
 *
 * Every exhibit computes into an output panel that starts empty, so a gate that
 * only scans first paint measures placeholders. Both verdict branches are driven
 * where a panel has them: a MAC that verifies and one that does not are
 * different surfaces with different palettes.
 */
export async function driveAllStates(page: Page, theme: string): Promise<void> {
  ...
  for (const mac of ['hmac', 'cmac', 'ghash', 'poly']) {
    await page.locator(`#${mac}-run`).click();
    await expect(page.locator(`#${mac}-output`)).not.toBeEmpty();
    await scan(page, `${theme} / ${mac} computed`);

    await page.locator(`#${mac}-verify`).click();
    await expect(page.locator(`#${mac}-verdict`)).not.toBeEmpty();
    await scan(page, `${theme} / ${mac} verified`);
  }

  // The two nonce/key-reuse attacks.
  await page.locator('#ghash-attack').click();
  await expect(page.locator('#ghash-attack-output')).not.toBeEmpty();
  await scan(page, `${theme} / GHASH nonce-reuse attack`);

  await page.locator('#poly-attack').click();
  await expect(page.locator('#poly-attack-output')).not.toBeEmpty();
  await scan(page, `${theme} / Poly1305 key-reuse attack`);

  // Length extension: capture a tag, forge, then both verifier verdicts.
  await page.locator('#le-capture').click();
  await expect(page.locator('#le-raw-tag')).not.toBeEmpty();
  await page.locator('#le-forge').click();
  await expect(page.locator('#le-forge-output')).not.toBeEmpty();
  await scan(page, `${theme} / length-extension forgery`);

  await page.locator('#le-verify-raw').click();
  await expect(page.locator('#le-verdict-raw')).not.toBeEmpty();
  await page.locator('#le-verify-hmac').click();
  await expect(page.locator('#le-verdict-hmac')).not.toBeEmpty();
  await scan(page, `${theme} / length-extension verdicts`);

  // The timing measurement table and the byte-recovery run.
  await page.locator('#timing-run').click();
  await expect(page.locator('#timing-summary')).not.toBeEmpty();
  await scan(page, `${theme} / timing measurements`);

  await page.locator('#recovery-run').click();
  await expect(page.locator('#recovery-output')).not.toBeEmpty();
  await scan(page, `${theme} / byte recovery`);

  // The guided tour overlay.
  await page.locator('#tour-start').click();
  await expect(page.locator('#tour-body')).toBeVisible();
  await scan(page, `${theme} / tour`);
  await page.locator('#tour-end').click();
}
