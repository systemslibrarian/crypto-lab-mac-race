 ...
 *    files the node under `incomplete`. The verdict chips and lane panels are
 *    `color-mix(...)` tints over the panel, so their text composites onto
 *    something axe will not measure.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. Note this lab applies its
 *    entrance animations only inside `@media (prefers-reduced-motion:
 *    no-preference)`, which is the safe pattern: under the preference nothing
 *    is animated and nothing strands at `opacity: 0`.
 *
 ...
 *    the MAC comparison and timing tables sit in `.table-wrap` scrollers and the
 *    `pre.src` code blocks scroll too, so at 380px much of them is clipped away.
